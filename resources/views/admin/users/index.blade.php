...
<div class="w-full min-w-0 max-w-full space-y-6">
        <div class="flex items-center justify-between flex-wrap gap-4 mb-6">
            ...
            @php
        $canShowAddUser = !empty($isSuperAdmin) || !empty($isCoordinatorManager);
        if (!$canShowAddUser && session('admin_user_id')) {
            $indexAdminUser = \App\Models\User::find(session('admin_user_id'));
            $canShowAddUser = $indexAdminUser && (
                $indexAdminUser->isSuperAdmin()
                || ($indexAdminUser->isCoordinator() && ! $indexAdminUser->isSuperAdmin())
            );
        }
        $addUserLabel = !empty($isCoordinatorManager) ? 'Add examiner' : 'Add user';
    @endphp
        @if($canShowAddUser)
        ...
        <div class="card overflow-hidden min-w-0">
            <div class="overflow-x-auto">
                <table class="w-full min-w-[600px] divide-y divide-gray-200">
                    ...
                    <tbody id="live-search-results" class="bg-white divide-y divide-gray-200">
                        @include('admin.users.partials.user-rows', [
                            'users' => $users,
                            'isSuperAdmin' => $isSuperAdmin ?? false,
                            'canManageAiTokens' => $canManageAiTokens ?? false,
                            'isCoordinatorManager' => $isCoordinatorManager ?? false,
                            'search' => $search ?? '',
                        ])
                    </tbody>
                </table>
            </div>
            <div id="live-search-pagination-wrap" class="{{ $users->hasPages() ? '' : 'hidden' }} px-6 py-4 border-t border-gray-200">
                <div id="live-search-pagination">@if($users->hasPages()){{ $users->links() }}@endif</div>
            </div>
        </div>
    </div>
